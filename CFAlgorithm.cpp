...
void CFAlgorithm::execute(){
    ...
    if (method==1){
        ...
        timeVector= timeVector+float(clock()-begin_time3);//Gets the time and stops counting for input
        ...
        //Calculate correlation with all the users and counting for MatrixVector
        const clock_t begin_time4=clock();//Continues getting time
        loop=1;
        for (std::map<int,User>::iterator it=allUsers.begin(); it!=allUsers.end(); ++it){ //iterate over allusers
                if ((it->first)!=user){ //except chosen user
        ...
        timeVector= timeVector+float(clock()-begin_time4);//Stops counting time, the rest is independent of matrix type
        ...
        timeVector=time2D+float(clock()-begin_time3);//Stops Counting time
        ...
        //Resumes counting time for MatrixVector
        ...
        timeVector=timeVector+float(clock()-begin_time4);
